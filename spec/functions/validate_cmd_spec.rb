...
describe 'validate_cmd', unless: Puppet::Util::Platform.windows? do
  ...
  describe 'signature validation' do
    ...
    it {
      ...
      expect(subject).to run.with_params([], '', '').and_raise_error(Puppet::ParseError, %r{content must be a string})
    }

    it {
      pending('should implement stricter type checking')
      expect(subject).to run.with_params('', [], '').and_raise_error(Puppet::ParseError, %r{checkscript must be a string})
    }

    it {
      pending('should implement stricter type checking')
      expect(subject).to run.with_params('', '', []).and_raise_error(Puppet::ParseError, %r{custom error message must be a string})
    }
  end
