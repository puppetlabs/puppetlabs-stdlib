...
describe Puppet::Parser::Functions.function(:deep_merge) do
  ...
  describe 'when calling deep_merge from puppet' do
    it "should not compile when no arguments are passed" do
      skip("Fails on 2.6.x, see merge bug #15912") if Puppet.version =~ /^2\.6\./
  ...
  describe 'when calling deep_merge on the scope instance' do
    ...
    it 'should be able to deep merge two hashes' do
      new_hash = scope.function_deep_merge([{'one' => '1', 'two' => ['1'], 'three' => { 'four' => '1' }}, {'two' => ['2'], 'three' => { 'five' => '2' }}])
      expect(new_hash['one']).to   eq('1')
      expect(new_hash['two']).to   eq(['1', '2'])
      expect(new_hash['three']).to eq({ 'four' => '1', 'five' => '2'})
    end

    it 'should deep merge multiple hashes' do
