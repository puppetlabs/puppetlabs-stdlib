Puppet::Type.type(:file_line).provide(:ruby) do
  ...
  def handle_create_with_match()
    ...
    if match_count > 1 && resource[:multiple].to_s != 'true'
     raise Puppet::Error, _("More than one line in file '#{resource[:path]}' matches pattern '#{resource[:match]}'")
    end
  ...
  def handle_create_with_after
    ...
    if count > 1 && resource[:multiple].to_s != 'true'
      raise Puppet::Error, _("#{count} lines match pattern '#{resource[:after]}' in file '#{resource[:path]}'.  One or no line must match the pattern.")
    end
