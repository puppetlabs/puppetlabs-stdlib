module Puppet::Parser::Functions
  ...
  newfunction(:validate_hash, :doc => _(<<-'ENDHEREDOC')) do |args|
    ...
    unless args.length > 0 then
      raise Puppet::ParseError, (_("validate_hash(): wrong number of arguments (#{args.length}; must be > 0)"))
    end

    args.each do |arg|
      unless arg.is_a?(Hash)
        raise Puppet::ParseError, (_("#{arg.inspect} is not a Hash.  It looks to be a #{arg.class}"))
      end
    end
