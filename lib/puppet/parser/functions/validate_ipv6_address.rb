module Puppet::Parser::Functions
  ...
  newfunction(:validate_ipv6_address, :doc => _(<<-ENDHEREDOC)
    ...
    unless args.length > 0 then
      raise Puppet::ParseError, (_("validate_ipv6_address(): wrong number of arguments (#{args.length}; must be > 0)"))
    end
    ...
    args.each do |arg|
      ...
      begin
        unless IPAddr.new(arg).ipv6?
          raise Puppet::ParseError, _("#{arg.inspect} is not a valid IPv6 address.")
        end
