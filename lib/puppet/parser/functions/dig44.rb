...
missing. And the fourth argument can set a variable path separator.
  eos
    ...
    raise(Puppet::ParseError, _("dig44(): Wrong number of arguments given (#{arguments.size} for at least 2)")) if arguments.size < 2

    data, path, default = *arguments

    unless data.is_a?(Hash) or data.is_a?(Array)
      raise(Puppet::ParseError, _("dig44(): first argument must be a hash or an array, given #{data.class.name}"))
    end

    unless path.is_a? Array
      raise(Puppet::ParseError, _("dig44(): second argument must be an array, given #{path.class.name}"))
    end
