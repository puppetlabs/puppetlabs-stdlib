module Puppet::Parser::Functions
  ...
  newfunction(:validate_integer, :doc => _(<<-'ENDHEREDOC')) do |args|
    ...
    raise Puppet::ParseError, _("validate_integer(): Wrong number of arguments; must be 1, 2 or 3, got %{num_args}") % { num_args: args.length } unless args.length > 0 and args.length < 4
    ...
    if args.length > 1
      ...
      if args.length > 2 and max == ''
        ...
        begin
          ...
          raise Puppet::ParseError, _("validate_integer(): Expected second argument to be unset or an Integer, got %{max}:%{max_class}") % { max: max, max_class: max.class }
        end
      end
    else
      max = nil
    ...
    if args.length > 2
      begin
        ...
        raise Puppet::ParseError, _("validate_integer(): Expected third argument to be unset or an Integer, got %{min}:%{min_class}") % { min: min, min_class: min.class }
      end
    else
      min = nil
    end
    ...
    if min and max and min > max
      raise Puppet::ParseError, _("validate_integer(): Expected second argument to be larger than third argument, got %{max} < %{min}") % { max: max, min: min }
    end
    ...
    validator = lambda do |num|
      # check input < max
      if max and num > max
        raise Puppet::ParseError, _("validate_integer(): Expected %{val} to be smaller or equal to %{max}, got %{val}.") % { val: input.inspect, max: max }
      end
      # check input > min (this will only be checked if no exception has been raised before)
      if min and num < min
        raise Puppet::ParseError, _("validate_integer(): Expected %{val} to be greater or equal to %{min}, got %{val}.") % { value: input.inspect, min: min }
      end
    end
    ...
    when Array
      ...
      input.each_with_index do |arg, pos|
        begin
          ...
          raise Puppet::ParseError, _("validate_integer(): Expected element at array position %{pos} to be an Integer, got %{arg_class}") % { pos: pos, arg_class: arg.class }
        end
      end
    # for the sake of compatibility with ruby 1.8, we need extra handling of hashes
    when Hash
      raise Puppet::ParseError, _("validate_integer(): Expected first argument to be an Integer or Array, got %{arg_class}") % { arg_class: input.class }
