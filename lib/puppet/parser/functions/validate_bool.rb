module Puppet::Parser::Functions
  ...
  newfunction(:validate_bool, :doc => _(<<-'ENDHEREDOC')) do |args|
    ...
    unless args.length > 0 then
      raise Puppet::ParseError, (_("validate_bool(): wrong number of arguments (#{args.length}; must be > 0)"))
    end

    args.each do |arg|
      unless function_is_bool([arg])
        raise Puppet::ParseError, (_("#{arg.inspect} is not a boolean.  It looks to be a #{arg.class}"))
      end
    end
